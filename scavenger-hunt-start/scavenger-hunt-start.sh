...
wait_for_containers () {
  while ! nc -zv 192.168.200.105 22 -w 1 &> /dev/null
  do
    printf "."
    sleep 1
    ((i++))
...
case $choice in
  1)
    ...
    if docker container inspect scavenger-hunt &> /dev/null
    then 
      if docker ps | grep scavenger-hunt
      then
        printf '\n%s\n\n' \
        ...
        docker container start scavenger-hunt
        wait_for_containers
        printf '\n%s\n\n' \
      ...
      if 
        docker network create \
          --driver=bridge scav-hunt-net --attachable \
          --subnet=192.168.200.0/24 >/dev/null
        docker run --name scavenger-hunt \
          -d \
          -p 15552:22 \
          --network=scav-hunt-net \
          --ip="192.168.200.105" \
          -e "TZ=America/New_York" \
          cyberxsecurity/scavenger-hunt >/dev/null
      then
        printf '\n%s\n\n' \
  ...
  2)
    ...
    then
      printf '\n%s\n\n' "Stopping web apps..."
      docker container stop scavenger-hunt
  ...
  3)
    ...
    then
      ...
      # docker container stop scavenger-hunt &> /dev/null
      docker container rm -f scavenger-hunt &> /dev/null
      docker network rm scav-hunt-net &> /dev/null
      # yes | docker network prune &> /dev/null
      printf '\n%s\n\n' "Removing web app container images..." 
      docker image rm cyberxsecurity/scavenger-hunt &> /dev/null
